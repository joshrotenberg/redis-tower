...
#[tokio::test]
async fn cover_sinterstore_basic() {
    let mut c = conn().await;
    let s1 = "cover:sets:sinterstore_basic:s1";
    let s2 = "cover:sets:sinterstore_basic:s2";
    let dst = "cover:sets:sinterstore_basic:dst";
    c.execute(Del::new(s1)).await.unwrap();
    c.execute(Del::new(s2)).await.unwrap();
    c.execute(Del::new(dst)).await.unwrap();
    c.execute(SAdd::members(s1, ["a", "b", "c"])).await.unwrap();
...
#[tokio::test]
async fn cover_sintercard_basic() {
    let mut c = conn().await;
    let s1 = "cover:sets:sintercard_basic:s1";
    let s2 = "cover:sets:sintercard_basic:s2";
    c.execute(Del::new(s1)).await.unwrap();
    c.execute(Del::new(s2)).await.unwrap();
    c.execute(SAdd::members(s1, ["a", "b", "c"])).await.unwrap();
    c.execute(SAdd::members(s2, ["b", "c", "d"])).await.unwrap();
...
#[tokio::test]
async fn cover_sintercard_limit() {
    let mut c = conn().await;
...
#[tokio::test]
async fn cover_sintercard_no_overlap() {
    let mut c = conn().await;
    let s1 = "cover:sets:sintercard_no_overlap:s1";
    let s2 = "cover:sets:sintercard_no_overlap:s2";
    c.execute(Del::new(s1)).await.unwrap();
    c.execute(Del::new(s2)).await.unwrap();
    c.execute(SAdd::members(s1, ["a", "b"])).await.unwrap();
    c.execute(SAdd::members(s2, ["c", "d"])).await.unwrap();
