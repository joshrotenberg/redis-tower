...
// -- TLS cluster tests --
//
// These spin up a TLS-enabled cluster automatically using a generated private
// CA and server certificates from redis-server-wrapper. No external endpoint
// or env vars are needed for local execution.
//
// Run with:
...
async fn ensure_tls_cluster() -> Option<TlsClusterFixture> {
    ...
    let certs = match redis_server_wrapper::tls::generate_test_certs(&certs_dir) {
        Ok(c) => c,
        Err(e) => {
            if std::env::var_os("REDIS_TEST_REQUIRE_TLS").is_some() {
                panic!("required TLS certificates could not be generated: {e}");
            }
            eprintln!(
                "skipping TLS tests: failed to generate certs: {e}; \
                         set REDIS_TEST_REQUIRE_TLS=1 to fail instead"
            );
            return None;
        }
    ...
    for (index, (&port, &cluster_port)) in ports[..3].iter().zip(&ports[3..]).enumerate() {
        ...
        let server = RedisServer::new()
            ...
            .cluster_node_timeout(5_000)
            .cluster_announce_ip("127.0.0.1")
            .cluster_announce_tls_port(port)
        ...
        match server {
            ...
            Err(error) if std::env::var_os("REDIS_TEST_REQUIRE_TLS").is_some() => {
                panic!("required TLS cluster node {port} could not start: {error}")
            }
            Err(error) => {
                eprintln!(
                    "skipping TLS tests: failed to start TLS cluster node {port}: \
                             {error}; set REDIS_TEST_REQUIRE_TLS=1 to fail instead"
                );
                return None;
            }
        }
    ...
    if let Err(error) = nodes[0].cli().cluster_create(&addrs, 0).await {
        if std::env::var_os("REDIS_TEST_REQUIRE_TLS").is_some() {
            panic!("required TLS cluster could not be formed: {error}");
        }
        eprintln!(
            "skipping TLS tests: failed to form TLS cluster: {error}; \
                     set REDIS_TEST_REQUIRE_TLS=1 to fail instead"
        );
        return None;
    }
