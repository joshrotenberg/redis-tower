...
// -- Destructive failover phases live in a separate binary --
//
// The tests that kill a sentinel and then the master (and the
// reconnect-after-failover check that depends on them) moved to
// `tests/sentinel_failover.rs`. They ran against this file's shared `OnceCell`
// topology and degraded it for anything that ran afterwards, which made the
// suite order-dependent (a latent flake under nextest or reordered execution).
// Keeping this binary purely non-destructive means every test here is robust to
// parallel and reordered execution. See #509.
