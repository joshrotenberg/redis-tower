...
/// while `max_us` is the largest HDR-reported run maximum.
#[derive(Clone, Debug)]
pub struct AggregatedReport {
    ...
    pub p999_us: f64,
    pub max_us: f64,
    /// The bounded repeated-run input retained for optional publication JSON.
...
mod tests {
    ...
    fn report(commands_per_sec: f64, p50: f64, total_commands: u64) -> BenchReport {
        BenchReport {
            ...
            total_commands,
            errors: 0,
            elapsed_secs: total_commands as f64 / commands_per_sec,
            commands_per_sec,
            p50_us: p50,
            p90_us: p50 * 2.0,
            p99_us: p50 * 3.0,
