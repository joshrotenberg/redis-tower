...
//! [`aggregate`].
//!
//! Every sentinel-bench client is async, so unlike `standalone-bench`/`cluster-bench`
//! there is no blocking-thread worker variant here.
...
mod tests {
    ...
    fn report(ops_per_sec: f64, p50: f64, total_ops: u64) -> BenchReport {
        BenchReport {
            client: ClientKind::SentinelClient,
