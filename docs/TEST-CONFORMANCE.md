...
The scoreboard-scoped packages compile **735 integration tests** across **41 test binaries**. **210** tests are marked `#[ignore]` because they need explicit infrastructure; workflow selectors, rather than the annotation alone, determine whether they run.

| Surface | Package | Binaries | Compiled | `#[ignore]` | Pull request | Scheduled | No workflow selector |
|---|---|---:|---:|---:|---:|---:|---:|
| Standalone | `redis-tower` | 30 | 507 | 0 | 507 | 229 | 0 |
...
| RESP decode copy/allocation | Exact test-only first-frame copied-byte counts; simple, mixed, retained-lifetime, and fragmented Criterion cases; instrumented comparison of historical, production, and split/shared ownership | Copied-byte unit contract and Criterion regression suite per PR; raw allocation artifact retained when ownership changes | [ownership decision](CODEC-DECODE-PERFORMANCE.md), [raw evidence](../conformance/codec-copy-evidence.json), [benchmark sources](../crates/redis-tower-protocol/benches) |
