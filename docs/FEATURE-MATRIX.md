...
| Distributed coordination primitives | fenced lock, leader election, expirable semaphore, countdown latch, and Redis-time GCRA -- [`redis-tower-primitives`](https://github.com/joshrotenberg/redis-tower/tree/main/crates/redis-tower-primitives/src) | no (external crates available) | no (external crates available) |
