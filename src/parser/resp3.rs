//! Zero-copy RESP3 parser and codec.
//!
//! This module provides parsing of RESP3 frames using `bytes::Bytes` for efficient, zero-copy operation.
//! It supports all RESP3 data types, including fixed-length and streaming variants.
//!
//! # Example
//!
//! ```rust
//! use redis_tower::parser::resp3::{parse_frame, Frame};
//! use bytes::Bytes;
//!
...
//!
//! ```rust
//! use redis_tower::parser::resp3::{Parser, Frame};
//! use bytes::Bytes;
//!
//! let mut parser = Parser::new();
//!
//! // Feed partial data
//! parser.feed(Bytes::from("+HELLO\r"));
//! assert_eq!(parser.next_frame(), None); // Incomplete frame
//!
...
//!
//! // There's still incomplete data for the second frame
//! assert_eq!(parser.next_frame(), None);
//!
//! // Feed the rest of the second frame
//! parser.feed(Bytes::from("\n"));
//! let frame2 = parser.next_frame().unwrap();
//! assert_eq!(frame2, Frame::SimpleString(Bytes::from("WORLD")));
//! ```
...
impl Parser {
    ...
    ///
    /// Returns `None` if there is not enough data to parse a complete frame.
    /// Otherwise, returns the parsed frame and removes the consumed data from the buffer.
    pub fn next_frame(&mut self) -> Option<Frame> {
        if self.buffer.is_empty() {
            return None;
        }

        // Use split() to transfer ownership without cloning
        // This is more efficient than clone().freeze()
        ...
        match parse_frame(bytes.clone()) {
            ...
            Err(ParseError::Incomplete) => {
                // Not enough data, restore buffer and wait
                self.buffer.unsplit(bytes.into());
                None
            }
            Err(e) => {
                // Protocol error: don't restore buffer, let it stay cleared
                eprintln!("RESP parse error: {:?}, clearing buffer", e);
                None
            }
        }
    }
...
pub enum Frame {
    ...
    /// # Example
    /// ```
    /// use redis_tower::parser::resp3::Frame;
    /// use bytes::Bytes;
    ///
...
/// Error type for parsing RESP3 frames.
#[derive(Debug, PartialEq)]
pub enum ParseError {
    /// Not enough data to parse a complete frame
    Incomplete,
    /// Invalid tag byte at the start of the frame
    InvalidTag(u8),
    /// Invalid length specification in a bulk string or other length-prefixed type
    BadLength,
    /// Error converting bytes to UTF-8
    Utf8Error,
    /// Invalid format in a frame
    InvalidFormat,
    /// Invalid boolean value (not 't' or 'f')
    InvalidBoolean,
    /// Invalid special float value (not 'inf', '-inf', or 'nan')
    InvalidSpecialFloat,
}
...
pub fn parse_frame(input: Bytes) -> Result<(Frame, Bytes), ParseError> {
    ...
    match tag {
        ...
        b'$' => {
            ...
            // empty or streaming-terminator
            if len == 0 {
                if rest.starts_with(b"\r\n") {
                    return Ok((Frame::BulkString(Some(Bytes::new())), rest.slice(2..)));
                } else {
                    return Ok((Frame::BulkString(Some(Bytes::new())), rest));
                }
            }
        ...
        b'*' => {
            ...
            for _ in 0..count {
                let (item, next_cursor) = parse_frame(cursor.clone())?;
                items.push(item);
                cursor = next_cursor;
            }
        ...
        b'%' | b'|' => {
            ...
            for _ in 0..count {
                let (key, cursor1) = parse_frame(cursor.clone())?;
                let (val, cursor2) = parse_frame(cursor1.clone())?;
                pairs.push((key, val));
                cursor = cursor2;
            }
            if tag == b'%' {
        ...
        b';' => {
            ...
            // Handle zero-length chunk (end of stream marker)
            if len == 0 {
                if rest.starts_with(b"\r\n") {
                    return Ok((Frame::StreamedStringChunk(Bytes::new()), rest.slice(2..)));
                } else {
                    return Ok((Frame::StreamedStringChunk(Bytes::new()), rest));
                }
            }
...
///
/// Parses a complete streaming sequence, accumulating chunks until termination.
///
...
/// ## Streaming String
/// ```rust
/// use redis_tower::parser::resp3::{parse_streaming_sequence, Frame};
/// use bytes::Bytes;
///
/// let data = Bytes::from("$?\r\n;4\r\nHell\r\n;6\r\no worl\r\n;1\r\nd\r\n;0\r\n");
...
pub fn parse_streaming_sequence(input: Bytes) -> Result<(Frame, Bytes), ParseError> {
    ...
    let (header, mut rest) = parse_frame(input.clone())?;
    ...
    match header {
        Frame::StreamedStringHeader => {
            ...
            loop {
                let (frame, new_rest) = parse_frame(rest.clone())?;
                rest = new_rest;
        ...
        Frame::StreamedMapHeader => {
            ...
            loop {
                ...
                match frame {
                    ...
                    key => {
                        let (value, newer_rest) = parse_frame(rest.clone())?;
                        rest = newer_rest;
...
/// Parse a single RESP2 frame from the provided `Bytes`.
///
/// Delegates to the RESP2 upstream parser and maps into our RESP3 `Frame` enum where possible.
pub fn parse_frame_resp2(input: Bytes) -> Result<(Frame, Bytes), ParseError> {
    if input.is_empty() {
        return Err(ParseError::Incomplete);
    }

    // Check for streaming variants which are not supported in RESP2
    if input.len() >= 3
        && (input[0] == b'$' || input[0] == b'*')
        && input[1] == b'?'
        && input[2] == b'\r'
    ...
    // For RESP2 we only support simple string, error, integer, bulk string, and array
    match input[0] {
        b'+' | b'-' | b':' | b'$' | b'*' => parse_frame(input),
        _ => Err(ParseError::InvalidTag(input[0])),
    }
}
...
mod resp2_tests {
    ...
    #[test]
    fn test_resp2_simple_string() {
        let input = Bytes::from("+OK\r\nrest");
        let (frame, rest) = parse_frame_resp2(input).unwrap();
    ...
    #[test]
    fn test_resp2_error() {
        let input = Bytes::from("-ERR test\r\nTAIL");
        let (frame, rest) = parse_frame_resp2(input).unwrap();
        assert_eq!(frame, Frame::Error(Bytes::from("ERR test")));
        assert_eq!(rest, Bytes::from("TAIL"));
    }

    #[test]
    fn test_resp2_integer() {
    ...
    #[test]
    fn test_resp2_bulk_string_some() {
        let input = Bytes::from("$3\r\nfoo\r\nB");
        let (frame, rest) = parse_frame_resp2(input).unwrap();
        assert_eq!(frame, Frame::BulkString(Some(Bytes::from("foo"))));
        assert_eq!(rest, Bytes::from("B"));
    }

    #[test]
    fn test_resp2_bulk_string_null() {
        let input = Bytes::from("$-1\r\nZ");
        let (frame, rest) = parse_frame_resp2(input).unwrap();
        assert_eq!(frame, Frame::BulkString(None));
        assert_eq!(rest, Bytes::from("Z"));
    }

    #[test]
    fn test_resp2_array_some() {
        let input = Bytes::from("*2\r\n+one\r\n:2\r\nR");
        let (frame, rest) = parse_frame_resp2(input).unwrap();
        if let Frame::Array(Some(items)) = frame {
            assert_eq!(items.len(), 2);
            assert_eq!(items[0], Frame::SimpleString(Bytes::from("one")));
            assert_eq!(items[1], Frame::Integer(2));
        } else {
            panic!("expected Some array");
        }
        assert_eq!(rest, Bytes::from("R"));
    }

    #[test]
    fn test_resp2_array_null() {
        let input = Bytes::from("*-1\r\nS");
        let (frame, rest) = parse_frame_resp2(input).unwrap();
        assert_eq!(frame, Frame::Array(None));
        assert_eq!(rest, Bytes::from("S"));
    }

    #[test]
    fn test_resp2_array_empty() {
    ...
    #[test]
    fn test_resp2_bulk_string_empty() {
        let input = Bytes::from("$0\r\n\r\nY");
        let (frame, rest) = parse_frame_resp2(input).unwrap();
        assert_eq!(frame, Frame::BulkString(Some(Bytes::new())));
        assert_eq!(rest, Bytes::from("Y"));
    }

    // Tests to verify RESP2 mode rejects RESP3-only types

    #[test]
    fn test_resp2_rejects_double() {
        let input = Bytes::from(",3.14\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_boolean() {
        let input = Bytes::from("#t\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_null() {
        let input = Bytes::from("_\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_big_number() {
        let input = Bytes::from("(12345678901234567890\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_verbatim_string() {
        let input = Bytes::from("=10\r\ntxt:hello\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_map() {
        let input = Bytes::from("%2\r\n+key1\r\n+val1\r\n+key2\r\n+val2\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_set() {
        let input = Bytes::from("~2\r\n+one\r\n+two\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_attribute() {
        let input = Bytes::from("|1\r\n+key\r\n+val\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_push() {
        let input = Bytes::from(">2\r\n+msg\r\n+data\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_blob_error() {
        let input = Bytes::from("!3\r\nerr\r\n");
        assert!(parse_frame_resp2(input).is_err());
    }

    #[test]
    fn test_resp2_rejects_stream_terminator() {
...
#[cfg(test)]
mod tests {
    use super::{Frame, ParseError, frame_to_bytes, parse_frame, parse_streaming_sequence};
    use bytes::Bytes;
    ...
    fn test_streaming_chunks_edge_cases() {
        ...
        // Test zero-length chunk without extra CRLF
